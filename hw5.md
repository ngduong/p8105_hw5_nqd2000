...
``` r
sim_results = 
  tibble(b1 = c(0, 1, 2, 3, 4, 5, 6)) %>%            #start with a 6x1 tibble with true set b1 values 
  mutate(
    output_lists = map(.x = b1,                      #set values to iterate over as b1
...
sim_results %>% 
  ...
  group_by(b1) %>%                        #group by true parameter
  summarize(pct_rej = 100*n()/100)%>%       #create variable pct
  ggplot(aes(x = factor(b1),              #set variables for x, y axes and by group (b1)
             y = pct_rej, 
             fill = b1)) + 
